...
if [[ $# -ne 1 ]]; then
  echo "usage: run_gcode_parser_integration <integration-binary>" >&2
  exit 2
fi

binary=$1

# The parser integration fixtures are owned by this native test and remain
...
cp "$fixture_dir/toplevel.py" "$workdir/toplevel.py"
