...
#define LINUXCNC_GRPC_GCODE_OPERATION_TYPES_HPP
...
struct Extents {
  ...
  void update(const Position& pos) { update(pos.x, pos.y, pos.z); }
...
inline OperationType getOperationType(const Operation& op) {
  return std::visit(
      [](const auto& o) -> OperationType {
        return std::remove_reference_t<decltype(o)>::type;
      },
      op);
}
...
#endif  // LINUXCNC_GRPC_GCODE_OPERATION_TYPES_HPP
