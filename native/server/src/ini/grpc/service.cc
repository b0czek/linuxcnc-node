...
class IniServiceImpl final : public linuxcnc::v1::IniService::CallbackService,
 ...
 public:
  explicit IniServiceImpl(std::shared_ptr<const ActiveIni> ini) : ini_(std::move(ini)) {}
...
std::unique_ptr<ManagedGrpcService> make_ini_service(std::shared_ptr<const ActiveIni> ini) {
