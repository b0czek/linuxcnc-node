...
int main(int argc, char* argv[]) {
  ...
  std::string error;
  if (!linuxcnc::server::parse_config(argc, argv, &config, &show_help,
                                      &error)) {
  ...
  if (config.ini_file.empty()) {
    std::cerr
        << "linuxcnc-grpc-server: --ini is required for the wire daemon\n";
    return 2;
  }
  if (config.nml_file.empty()) {
    std::cerr
        << "linuxcnc-grpc-server: --nml is required for the wire daemon\n";
    return 2;
  }
  if (!std::filesystem::is_regular_file(config.nml_file)) {
    std::cerr
        << "linuxcnc-grpc-server: NML configuration is not a regular file: "
        << config.nml_file << '\n';
    return 2;
  }
#endif
  if (!config.ini_file.empty()) {
    if (!linuxcnc::server::validate_program_prefix(
            config.ini_file, config.active_program_directory, &error)) {
      std::cerr << "linuxcnc-grpc-server: " << error << '\n';
      return 2;
    }
  }
