...
std::vector<std::uint8_t> encode_position_telemetry_frame(
  ...
  if (batch.packed.size() > std::numeric_limits<std::uint32_t>::max()) {
    throw std::length_error("position telemetry frame exceeds uint32 value count");
  }
  std::vector<std::uint8_t> output(
      kPositionTelemetryHeaderSize + batch.packed.size() * sizeof(double));
  output[0] = 'L'; output[1] = 'C'; output[2] = 'P'; output[3] = 'H';
  output[4] = kPositionTelemetryVersion;
  output[5] = static_cast<std::uint8_t>(kind);
  write_le<std::uint16_t>(&output, 6, static_cast<std::uint16_t>(kPositionStride));
  ...
  for (std::size_t index = 0; index < batch.packed.size(); ++index) {
    ...
    write_le<std::uint64_t>(&output,
                            kPositionTelemetryHeaderSize + index * sizeof(double),
                            bits);
  }
  return output;
}
