...
try {
  ...
  for (let i = 0; i < expected.length; i += 1) {
    ...
    if (!expectedBytes.equals(actualBytes))
      throw new Error(`generated output differs: ${expectedNames[i]}`);
  }

  const expectedBarrel =
    expectedNames
      .filter((file) => file !== "health.ts")
      .map(
        (file) =>
  ...
  if (actualBarrel !== expectedBarrel)
    throw new Error("generated output differs: index.ts");
  console.log(
    `generated grpc output is deterministic (${expected.length} files)`,
  );
} finally {
