...
export async function createLinuxCncClients(options: LinuxCncClientOptions): Promise<LinuxCncClients> {
  const protoRoot = options.protoRoot ?? `${__dirname}/../proto`;
  const protoPath = options.protoPath ?? `${protoRoot}/linuxcnc/v1/linuxcnc.proto`;
  const healthProtoPath = options.healthProtoPath ?? `${protoRoot}/grpc/health/v1/health.proto`;
  const packageDefinition = await protoLoader.load([protoPath, healthProtoPath], {
  ...
  const loaded = grpc.loadPackageDefinition(packageDefinition) as unknown as ProtoGrpcType & HealthProtoGrpcType;
  const v1 = loaded.linuxcnc.v1;
  const credentials = options.credentials ?? grpc.credentials.createInsecure();
  return {
    machine: new v1.MachineService(options.address, credentials, options.channelOptions),
    program: new v1.ProgramService(options.address, credentials, options.channelOptions),
    hal: new v1.HalService(options.address, credentials, options.channelOptions),
    scope: new v1.ScopeService(options.address, credentials, options.channelOptions),
    health: new loaded.grpc.health.v1.Health(options.address, credentials, options.channelOptions),
  };
}
