...
const prefixFor = (name) => name.replace(/[A-Z]/g, (letter, index) => `${index ? "_" : ""}${letter}`).toUpperCase();
const parse = (name) => {
...
const parseOneofFields = (message, oneof) => {
  ...
  const match = body.match(new RegExp(`oneof\\s+${oneof}\\s*\\{([\\s\\S]*?)\\n\\s*\\}`));
  ...
  for (const [, wireType, wireName, rawNumber] of match[1].matchAll(
    ...
    fields.push({ name: snakeToCamel(wireName), wireName, number: Number(rawNumber), wireType });
  }
  return fields;
};
const domainMessages = [
  "Position", "ToolEntry", "ActiveGCodes", "ActiveMCodes", "ActiveSettings", "TaskStat",
  "JointStat", "AxisStat", "SpindleStat", "TrajectoryStat", "MotionStat", "ToolIoStat",
  "CoolantIoStat", "IoStat", "LinuxCNCStat", "LinuxCNCError", "Extents", "GCodeOperation",
  "HalItemRef", "HalScalar", "HalComponentInfo", "HalFunctionInfo", "HalThreadInfo",
  "HalPinInfo", "HalParamInfo", "HalSignalInfo", "HalTopology", "ScopeChannelConfig",
  "ScopeAcquisitionConfig", "ScopeStatus", "ScopeCapture", "ScopeCaptureDelta", "PackedChannel",
];
const domainFields = Object.fromEntries(domainMessages.map((name) => [name, parseMessageFields(name)]));
...
let output = "/** Generated from proto/linuxcnc/v1/linuxcnc.proto. Do not edit manually. */\n\n";
for (const name of stableEnums) {
  output += `export enum ${name} {\n`;
  for (const [member, value] of parse(name)) output += `  ${member} = ${value},\n`;
  output += "}\n\n";
}
...
let domainOutput = "/** Generated from proto/linuxcnc/v1/linuxcnc.proto. Do not edit manually. */\n\n";
...
for (const field of gcodeOperationVariants) domainOutput += `  { name: "${field.name}", wireName: "${field.wireName}", number: ${field.number} },\n`;
domainOutput += "] as const;\n\n";
domainOutput += "export const PROTO_HAL_SCALAR_VARIANTS = [\n";
for (const field of halScalarVariants) domainOutput += `  { name: "${field.name}", wireName: "${field.wireName}", number: ${field.number} },\n`;
domainOutput += "] as const;\n\n";
domainOutput += "export const PROTO_PACKED_CHANNEL_FIELDS = [\n";
for (const field of packedChannelFields) domainOutput += `  { name: "${field.name}", wireName: "${field.wireName}", number: ${field.number} },\n`;
domainOutput += "] as const;\n";
writeFileSync(generatedDomain, domainOutput);

const candidates = [process.env.PROTOC, "protoc"].filter(Boolean);
const protoc = candidates.find((candidate) => spawnSync(candidate, ["--version"], { encoding: "utf8" }).status === 0);
if (protoc) {
  const descriptorDir = mkdtempSync(join(tmpdir(), "linuxcnc-types-descriptor-"));
  try {
    const descriptor = join(descriptorDir, "linuxcnc-v1.generated.pb");
    const result = spawnSync(protoc, ["-I", join(root, "proto"), "-I", "/usr/include", `--descriptor_set_out=${descriptor}`, proto], { encoding: "utf8" });
    if (result.status !== 0) throw new Error(result.stderr || "protoc failed");
    console.log(`generated enums and validated protobuf descriptor with ${protoc}`);
  } finally {
    rmSync(descriptorDir, { recursive: true, force: true });
  }
} else {
  console.warn("protoc is not installed; generated enums from schema text and skipped descriptor validation.");
}
