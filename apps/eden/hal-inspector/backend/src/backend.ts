...
const defaultScopeStatus = (): ScopeStatus => ({
  state: "idle",
  bufferLength: 0,
  recordLength: 0,
  sampleLength: 0,
    ...
    const text =
      typeof input === "number"
        ...
        : typeof input === "string"
          ...
          : "";
    if (!/^[+-]?\d+$/.test(text))
      throw new Error(`${type} requires a decimal integer`);
    const value = BigInt(text);
    const min = type === "s64" ? -(1n << 63n) : 0n;
    const max = type === "s64" ? (1n << 63n) - 1n : (1n << 64n) - 1n;
    if (value < min || value > max)
      throw new Error(`Value is outside the ${type} range`);
    return value;
  }
...
async function refreshTopology(send = false): Promise<TopologySnapshot> {
  ...
  const shape = (value: TopologySnapshot) =>
    JSON.stringify({
      ...
      p: value.pins.map(({ name, type, direction, signalName }) => [
        name,
        type,
        direction,
        signalName,
      ]),
      a: value.params.map(({ name, type, direction }) => [
        name,
        type,
        direction,
      ]),
      s: value.signals.map(({ name, type, readers, writers, bidirs }) => [
        name,
        type,
        readers,
        writers,
      ...
      f: value.functions.map(({ name, ownerId, users }) => [
        name,
        ownerId,
        users,
      ]),
      t: value.threads.map(
        ({ name, periodNs, priority, running, functions }) => [
          name,
          periodNs,
          priority,
          running,
  ...
  if (send && (!previous || shape(previous) !== shape(topology)))
...
function deliverCapture(capture: ScopeCapture): void {
  ...
  const id = ++captureId;
  inFlightCapture = id;
  inFlightGeneration = capture.generation;
...
function mergeRollBatches(
  ...
  return {
    ...
    channels: next.channels.map((channel, index) => {
      ...
      merged.set(
        channel.subarray(Math.max(0, channel.length - (kept - beforeKept))),
        beforeKept,
      );
      return merged;
    }),
...
function deliverRoll(batch: ScopeCaptureDelta): void {
  ...
  const id = ++captureId;
  inFlightCapture = id;
  inFlightGeneration = batch.generation;
...
async function ensureScope(): Promise<ScopeSession> {
  ...
  scopeOff = session.onMessage((message) => {
    if (message.status)
      api.send("scope/status", mapScopeStatus(message.status));
...
api.handle("bootstrap/get", async () => {
...
api.handle("topology/refresh", async () => {
  if (!connected) return fail("DISCONNECTED", startupError);
  try {
    return ok(await refreshTopology());
  } catch (error) {
    return fail("DISCONNECTED", error);
  }
});
  ...
  try {
    ...
    const writable =
      ref.kind === "param"
        ? meta.direction === "rw"
        : ref.kind === "pin"
          ? meta.direction !== "out" && !meta.signalName
          : meta.writers === 0;
    if (!writable)
...
api.handle("scope/ensure", () =>
  queued(async () => {
    try {
      return ok((await ensureScope()).status ?? defaultScopeStatus());
    } catch (error) {
...
api.handle("scope/configure", (config: ScopeAcquisitionConfig) =>
  queued(async () => {
    try {
      const session = await ensureScope();
      ...
      const current = session.status ?? defaultScopeStatus();
      await session.send(scopeConfigure(wireScopeConfig(config)));
      if (wasRolling) {
        pendingCapture = null;
        pendingRoll = null;
        skippedCaptures = 0;
        ...
        await session.send(
          scopeConfigure(
            wireScopeConfig(rollConfig(scopeConfig, current.recordLength)),
          ),
        );
        await session.send(scopeRun(3));
        return ok(session.status ?? { ...current, state: "init" });
      }
      if (wasRunning) await session.send(scopeRun(1));
      if (wasSingle) setRunMode("stop");
      return ok(session.status ?? current);
    } catch (error) {
      setRunMode("stop");
      return fail(classifyGrpcError(error), error);
    }
...
api.handle("scope/run", ({ mode }) =>
  queued(async () => {
    try {
      ...
      const config =
        mode === "roll"
          ? rollConfig(scopeConfig, current.recordLength)
          : scopeConfig;
      await session.send(scopeConfigure(wireScopeConfig(config)));
      if (mode === "roll") rollGeneration++;
      await session.send(
        scopeRun(mode === "run" ? 1 : mode === "single" ? 2 : 3),
      );
      setRunMode(mode);
      return ok(session.status ?? current);
    } catch (error) {
...
api.handle("scope/stop", () =>
  queued(async () => {
    try {
      ...
      await scope.send(scopeStop());
      setRunMode("stop");
      pendingRoll = null;
      return ok(scope.status ?? defaultScopeStatus());
    } catch (error) {
      return fail(classifyGrpcError(error), error);
    }
  }),
  ...
  queued(async () => {
    try {
      ...
      if (runMode === "roll")
        return fail(
          "INVALID_VALUE",
          "Force trigger is unavailable in Roll mode",
        );
      await scope.send(scopeTrigger());
      return ok(scope.status ?? defaultScopeStatus());
    } catch (error) {
      return fail(classifyGrpcError(error), error);
    }
...
api.on("scope/capture-ack", ({ id }) => {
  ...
  void scope
    ?.send(scopeAck(inFlightGeneration ?? BigInt(id)))
    .catch(() => undefined);
